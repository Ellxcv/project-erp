...
class BomController extends Controller
{
    ...
    {
        $request->validate([
            'kode_produk' => 'required|integer|exists:t_produk,id',
            'qty'         => 'required|numeric|min:0.0001',
            'satuan'      => 'required|string',
        ], [
            'kode_produk.required' => 'Harap pilih bahan terlebih dahulu.',
            'kode_produk.integer'  => 'Pilihan bahan tidak valid.',
            'kode_produk.exists'   => 'Bahan yang dipilih tidak ditemukan.',

            'qty.required'         => 'Harap isi banyak bahan.',
            'qty.numeric'          => 'Banyak bahan harus berupa angka.',
            'qty.min'              => 'Banyak bahan harus lebih dari 0.',

            'satuan.required'      => 'Harap pilih satuan bahan.',
        ]);
        ...
        $bom->total_harga = $bom->total_harga + ($harga * $request->qty);
        $bom->save();
